...
public class QnaDAO {
	...
	public int delQuestion(int parno){
		...
		try {
			...
			if(cnt>0) {
				pstmt = con.prepareStatement(SqlLang.GET_QUESTIONS_AFTER_DELETED);
				pstmt.setInt(1, parno);
				ResultSet rs = pstmt.executeQuery();
				int offset = 1;
				while(rs.next()) {
					int oldQuestionNo = rs.getInt("question_no");
					int newQuestionNo = oldQuestionNo - offset;
					pstmt = con.prepareStatement(SqlLang.UPDATE_QUESTION_NO);
					pstmt.setInt(1, newQuestionNo);
					pstmt.setInt(2, oldQuestionNo);
					pstmt.executeUpdate();
					offset++;
				}
			}
